                                        ...
                                        <div class="onhover-dropdown">
                                            ...
                                            <div class="onhover-div">
                                                <ul class="notification-list">
                                                    @php
                                                        $thongBaos = Auth::check() ? App\Models\ThongBao::with('danhGia.sanPham')->where('user_id', Auth::id())->where('trang_thai', 0)->get() : collect([]);
                                                    @endphp
                                                    @if ($thongBaos->count() > 0)
                                                        @foreach ($thongBaos as $thongBao)
                                                            <li class="notification-box" id="notification-{{ $thongBao->id }}">
                                                                <div class="notification-content">
                                                                    <h6>{{ $thongBao->noi_dung }}</h6>
                                                                    <p>Sản phẩm: {{ $thongBao->danhGia->sanPham->ten_san_pham ?? 'Không xác định' }}</p>
                                                                    ...
                                                                    <button class="btn btn-sm btn-link mark-as-read" data-id="{{ $thongBao->id }}" data-url="{{ route('thongbao.da_doc', $thongBao->id) }}">Đánh dấu đã đọc</button>
                                                                </div>
                                                            </li>
                                                        @endforeach
                                                    @else
                                        ...
                                        <div class="onhover-dropdown header-badge">
                                            ...
                                            <div class="onhover-div">
                                                            ...
                                                            <div class="drop-cart">
                                                                ...
                                                                <div class="dropcontain">
                                                                    ...
                                                                    <h6><span>{{ $item->so_luong }} x</span>
                                                                        {{ number_format($item->bienThe->gia_ban, 0, '', '.') }} đ
                                                                    </h6>
                                                                    <style>
                                                                        .hidden-delete {
                                                                            visibility: hidden;
                                                                    ...
                                                                    @if (!request()->is('giohang', 'thanhtoan'))
                                                                        <button class="close-button close_button delete-cart-item"
                                                                            data-id="{{ $item->id }}">
                                                                            <i class="fa-solid fa-xmark"></i>
                                                                        </button>
                                                                    @else
                                                                        <button class="close-button close_button delete-cart-item hidden-delete"
                                                                            data-id="{{ $item->id }}">
                                                                            <i class="fa-solid fa-xmark"></i>
                                                                        </button>
                                                                    @endif
                                                ...
                                                <div class="button-group">
                                                    <a href="{{ route('giohang') }}" class="btn btn-sm cart-button">Xem giỏ hàng</a>
                                                    <a href="{{ route('thanhtoans.thanhtoan') }}"
                                                        class="btn btn-sm cart-button theme-bg-color text-white">Thanh toán</a>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                    ...
                    if (data.success) {
                        ...
                        if (badge) {
                            ...
                            if (count > 0) {
                                ...
                                badge.style.display = 'none'; // Ẩn số lượng, giữ icon chuông
                            }
                        }
